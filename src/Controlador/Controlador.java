...
import Services.ResolucionImpl;
...
public class Controlador implements ActionListener {
    ...
    public Controlador() {
        ...
        resolDAO = new ResolucionImpl();
        bene = new Beneficiario();
        beneDAO = new BeneficiarioImpl();
        frR = new FrmResumen();
        frU = new FrmUsuarios();
        usr = new User();
        usrDAO = new UserImpl();
        resumenDAO= new ResumenImpl();
    }
    ...
    public void actionPerformed(ActionEvent e) {
        switch (e.getActionCommand()) {
            ...
            case "Por Mes" -> {
                ...
                llenarTablaResumen(resumenDAO.selectPorMes());
            }
            case "Totales" -> {
                frP.setExtendedState(Frame.MAXIMIZED_BOTH);
     ...
     * @param rptImprimir
     */
    private void imprimir(String rptImprimir) {
     ...
     */
    private void llenarTabla(ArrayList<Resolucion> listado) {

        DefaultTableModel tabla;
        tabla = (DefaultTableModel) frC.tabla.getModel();
        limpiarTabla(tabla);
        String[] camposResol = new String[12];
        for (Resolucion res : listado) {
            ...
            camposResol[10] = String.valueOf(res.getDepto_origen());
            camposResol[11] = String.valueOf(res.getMunicipio_origen());
            tabla.addRow(camposResol);
        }
    }

    private void llenarTablaUsuarios(ArrayList<User> listado) {
     ...
     * @param listado - permite saber con que datos se llena dicha tabla
     */
    private void llenarTablaBene(ArrayList<Beneficiario> listado) {
        limpiarTabla((DefaultTableModel) frC.tablab.getModel());
        //ResolucionJDBC resJDBC = new ResolucionImpl();
        DefaultTableModel tabla;
        tabla = (DefaultTableModel) frC.tablab.getModel();
        String[] camposBene = new String[5];
        for (Beneficiario bene : listado) {
            camposBene[0] = String.valueOf(bene.getResolucion_id());
            camposBene[1] = String.valueOf(bene.getTipo_documento());
            camposBene[2] = String.valueOf(bene.getId_beneficiario());
            camposBene[3] = String.valueOf(bene.getNombre_beneficiario());
            camposBene[4] = String.valueOf(bene.getApellido_beneficiario());
     ...
     */
    private void llenarTablaResumen(ArrayList<Resumen> listado) {
    ...
    private double[] calcularTotales(ArrayList<Resumen> listado) {
        //limpiarTabla();
        //ResolucionJDBC resJDBC = new ResolucionImpl();
        //DefaultTableModel tabla;
        //tabla = (DefaultTableModel) frR.tablaresumen.getModel();
...
//      frC.txtidresolucion.setText("");
        frC.cbotipodocumento.setSelectedItem(null);
...
//    }
    ...
    private void deletereg() {
        ...
        ResolucionImpl resDAO = new ResolucionImpl();
    ...
    private void deleteregU() {
        // Procesa Registros de Resoluciones
        UserImpl usrDAO = new UserImpl();
        User usr = new User();
    ...
    private void eliminaregbene() {
        ...
        bene.setTipo_documento(String.valueOf(frC.tablab.getValueAt(fila, 1)));
        bene.setId_beneficiario(String.valueOf(frC.tablab.getValueAt(fila, 2)));
        bene.setNombre_beneficiario(String.valueOf(frC.tablab.getValueAt(fila, 3)));
        bene.setApellido_beneficiario(String.valueOf(this.frC.tablab.getValueAt(fila, 4)));
        beneDAO.delete(bene);
    }
    ...
    private void limpiarCampos() {
        ...
        frC.cbodepto.setSelectedItem(null);
        frC.cbomunicipio.setSelectedItem(null);
        frC.txtbuscar.setText("");

    }

    private void limpiarCamposU() {
    ...
    private void agregar() {
        if (frC.jdcfresol.getDate().toString().isEmpty() || frC.txtbeneficiarias.getText().isEmpty()
            ...
            resol.setDepto_origen(frC.cbodepto.getSelectedItem().toString());
            resol.setMunicipio_origen(frC.cbomunicipio.getSelectedItem().toString());
    ...
    private void update() {
        if (frC.jdcfresol.getDate().toString().isEmpty() || frC.txtbeneficiarias.getText().isEmpty() || Double.parseDouble(frC.txtmonto.getText()) <= 0) {
            JOptionPane.showMessageDialog(frC, "No se admiten campos en blanco");
        } else {
            resolDAO = new ResolucionImpl();
            ...
            resol.setNum_resolucion(frC.txtnumresolucion.getText());
            resol.setFecha_resolucion(Funciones.convertirDateASql(frC.jdcfresol.getDate()));
            //resol.setFecha_resolucion(fechaAString( frC.jdcfresol.getDate()));
            resol.setFam_beneficiarias(Integer.parseInt(frC.txtbeneficiarias.getText()));
            resol.setMonto_resolucion(Double.parseDouble(frC.txtmonto.getText()));
            resol.setEmitida_por(frC.txtemitidapor.getText());
            //resol.setFecha_registro(cadenaAfecha(frC.jdcfregistro.getDate().toString()));
            resol.setFecha_registro(Funciones.convertirDateASql(frC.jdcfregistro.getDate()));
            resol.setDepto_origen(frC.cbodepto.getSelectedItem().toString());
            resol.setMunicipio_origen(frC.cbomunicipio.getSelectedItem().toString());
    ...
    private void updateBene() {
        if (this.frC.cbotipodocumento.getSelectedItem().toString().isEmpty()
                || this.frC.txtnombrebeneficiario.getText().isEmpty() || this.frC.txtapellidobeneficiario.getText().isEmpty()) {
            JOptionPane.showMessageDialog(frC, "No se admiten campos en blanco");
        } else {
            beneDAO = new BeneficiarioImpl();
            bene.setTipo_documento(this.frC.cbotipodocumento.getSelectedItem().toString());
            ...
            String strbene = String.valueOf(this.frC.tablab.getValueAt(filasect, 2));
    ...
    private void addBene() {
        if (frC.txtnumresolucionb.getText().isEmpty() || frC.cbotipodocumento.getSelectedItem().toString().isEmpty()
            ...
            bene.setTipo_documento(frC.cbotipodocumento.getSelectedItem().toString());
            bene.setId_beneficiario(frC.cbotipodocumento.getSelectedItem().toString());
    ...
    private void rptTotales() {
        String reporte = "RptResumen.jasper"; //1.Limpiamos la ventana antes de mostrar nada
        limpiarTabla((DefaultTableModel) frC.tabla.getModel());
        ArrayList<Resolucion> listaresoluciones;
    ...
    private void eliminarBene() {
        if (frC.cbotipodocumento.getSelectedItem().toString().isEmpty() || frC.txtnombrebeneficiario.getText().isEmpty() || frC.txtapellidobeneficiario.getText().isEmpty()) {
            JOptionPane.showMessageDialog(frC, "No se admiten campos en blanco");
            JOptionPane.showMessageDialog(frC, "Debe seleccionar un registro");
    ...
    private void porfecha() {
        ...
        llenarTablaResumen(resumenDAO.selectPorFecha());
        double[] arrResumen = new double[3];
        arrResumen = calcularTotales(resumenDAO.selectPorFecha());
